...
public class UploadServlet extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (Part part : parts) {
			if (part.getContentType() != null) {	// 업로드되는 파일인 애들은 getContentType이라는것이 존재 하기떄문에 이것으로 파일인지 아닌지 구분하는 것이다.
				String fileName = part.getSubmittedFileName();	// 실제 업로드 되는 파일명을 꺼내는 메서드이다.
				if (fileName != null) {
